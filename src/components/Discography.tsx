        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {albums.map((album, i) => (
            <motion.div
              ...
              className="group relative aspect-square rounded-3xl overflow-hidden glass border-white/10"
            >
              <img 
                src={album.img} 
                alt={album.title}
              ...
              />
              <div className="absolute inset-0 bg-gradient-to-t from-eternal-black via-eternal-black/20 to-transparent opacity-60 group-hover:opacity-90 transition-opacity" />
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center p-6 opacity-0 group-hover:opacity-100 transition-all duration-500 translate-y-4 group-hover:translate-y-0">
                <span className="text-eternal-purple font-black text-4xl mb-2">#{album.year}</span>
                <h3 className="text-xl font-display font-bold text-center text-white mb-4">{album.title}</h3>
